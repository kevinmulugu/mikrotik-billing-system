    ...
    try {
      ...
      if (!signupResponse.ok) {
        if (signupResponse.status === 409) {
          ...
          toast.error('Signup failed', {
            description: signupData.error || 'Please try again.',
      ...
      if (result?.error) {
        toast.error('Failed to send verification email', {
          description: 'Account created but email failed. Please try signing in.',
          icon: <XCircle className="w-4 h-4" />,
        });
      } else {
      ...
      <Card className="max-w-md w-full">
        <CardHeader className="text-center space-y-2">
          ...
          <CardDescription className="text-base">
            Create your PAY N BROWSE account
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          {configError && (
            <Alert variant="destructive">
              <AlertDescription>
                Unable to sign up at this time. Please try again later or contact support if the problem persists.
              </AlertDescription>
            </Alert>
          )}
